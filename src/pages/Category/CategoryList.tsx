...
export const CategoryList = memo(() => {
  ...
  const categoryItems = category.map((categoryItem) => {
    return (
      <CardItem
        key={categoryItem.idMeal}
        title={categoryItem.strMeal}
        btnText={'Watch recipe'}
        link={`/meal/${categoryItem.idMeal}`}
        image={categoryItem.strMealThumb}
      />
    )
  })

  return <List>{!isLoading ? categoryItems : <SkeletList amount={12} />}</List>
})
