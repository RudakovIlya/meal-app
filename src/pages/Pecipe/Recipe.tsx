...
const Recipe = memo(() => {
  ...
  return (
    <>
      {!isLoading ? (
        <>
          <Grid sx={{ mb: 2 }} columnSpacing={{ xs: 1, sm: 2, md: 3 }} container columns={{ xs: 4, sm: 8, md: 12 }}>
            ...
            <Grid item xs={12} sm={12} md={6}>
              ...
              <Typography component='p'>{recipe?.strInstructions}</Typography>
            </Grid>
          </Grid>
          <TableContainer sx={{ mb: 4 }} component={Paper} elevation={3}>
            <Table aria-label='simple table'>
              ...
              <TableBody>
                {recipe &&
                  (Object.keys(recipe) as string[]).map((key: string) => {
                    if (key.includes('Ingredient') && recipe[key as keyof RecipeType]) {
                      return (
                        <TableRow key={key}>
                          <TableCell align='left'>{recipe[key as keyof RecipeType]}</TableCell>
